      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (dist < MAX_DIST) {
            ...
            ctx.strokeStyle = `rgba(240,240,250,${alpha * 1.8})`
            ctx.lineWidth = 0.5
            ctx.stroke()
          }
        }
...
const SCIENCE_LABELS: [string, number, number, number, number, number, number, number][] = [
  // ── Physics ──
  ['E = mc²',            310,  78, 11, 0, 0.80, 18,  0.0],
  ['F = ma',             460,  62, 10, 0, 0.72, 20,  3.8],
  ['ΔxΔp ≥ ℏ/2',        440, 148,  9, 0, 0.74, 17,  8.2],
  ['iℏ∂ψ/∂t = Ĥψ',     290, 205,  9, 0, 0.74, 22,  1.5],
  ['∇·E = ρ/ε₀',        470, 295, 10, 0, 0.70, 16, 12.0],
  ['c = λf',             130, 430, 10, 0, 0.74, 15,  6.5],
  ['T = 2π√(L/g)',        80,  85,  9, 0, 0.68, 21, 15.0],
  ['pV = nRT',           460, 530, 10, 0, 0.74, 19,  0.8],
  ['S = k ln Ω',         155, 560, 10, 0, 0.70, 23,  9.3],
  ['a = Gm/r²',          300, 485, 10, 0, 0.74, 17,  4.1],
  ['ℏ',                  405, 525, 13, 0, 0.82, 15, 11.7],
  // ── Mathematics ──
  ['e^(iπ) + 1 = 0',     410, 170, 11, 0, 0.84, 24,  2.0],
  ['∑ n=1→∞ 1/n²',      355, 198,  9, 0, 0.70, 20,  7.5],
  ['∂f/∂x',             480, 185, 10, 0, 0.74, 16, 13.5],
  ['∇²ψ = 0',           100, 505, 10, 0, 0.70, 18,  5.0],
  ['O(n log n)',          88, 525,  9, 0, 0.68, 22, 10.2],
  ['P(A|B)',             150, 485, 10, 0, 0.74, 19,  1.3],
  ['π ≈ 3.14159',       470,  60,  9, 0, 0.68, 16, 14.8],
  ['∫₋∞^∞ e^−x² dx',    285,  62,  9, 0, 0.68, 21,  6.0],
  ['lim x→0 sin(x)/x',  460, 390,  9, 0, 0.68, 17,  2.9],
  // ── Genetics & RNA ──
  ['5′-AUGCGA-3′',       310, 600,  9, 0, 0.74, 20, 11.0],
  ['A-T · G-C',          420, 620,  9, 0, 0.70, 18,  0.5],
  ['mRNA → protein',     230, 510,  9, 0, 0.68, 22,  8.0],
  ['UGA (stop)',          140, 545,  9, 0, 0.68, 15, 15.5],
  ['CRISPR-Cas9',        200, 625,  9, 0, 0.74, 19,  3.5],
  ['p53 ⊣ apoptosis',     95, 570,  8, 0, 0.65, 23,  7.0],
  ['ATCG',               455, 455, 10, 0, 0.70, 16,  1.0],
  // ── Chemistry ──
  ['ΔG = ΔH − TΔS',     385, 460, 10, 0, 0.74, 18,  9.7],
  ['CH₄ + 2O₂ →',      340, 640,  9, 0, 0.68, 20, 13.0],
  ['pH = −log[H⁺]',     255, 650,  9, 0, 0.68, 21,  4.8],
  ['Kₑ=[C][D]/[A][B]',  110, 610,  8, 0, 0.65, 24,  0.2],
  ['ATP → ADP + Pᵢ',    470, 355,  9, 0, 0.68, 17, 12.3],
  ['6CO₂+6H₂O→C₆H₁₂O₆',380,580,  8, 0, 0.62, 23,  5.7],
]
